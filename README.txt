...
συνάρτησης «int jobCommander(int argc, char *argv[])».
    ...
    - Διάβαζει το μήνυμα ή και το output του job, που επέστρεψε ο Server.

3. Το αρχείο jobExecutorServer.c είναι υπέυθυνο για τις συνδέσεις του Server με τους Commanders
και την αποθήκευση/εκτέλεση των jobs που στέλνουν.
    - Δημιουργεί το buffer (Queue) που αποθηκεύονται οι εργασίες.
    - Αρχικοποιεί το struct που κρατάει τις πληροφορίες του Server και το συδέει με ένα global pointer.
    ...
    - Διαχείριζεται τη διαδικασία του κλεισίματος του Server.
...
4. Η διαχείρηση των commands από τον Commander προς τον Server γίνεται με τα αρχεία:
    ...
    τις δήλωσεις των συναρτήσεων για κάθε command, δύο βοηθητικές συναρτήσεις για τα worker και 
    ...
    4.3. Η συνάρτηση «char* stop_job()»:
        ...
        - Αφαίρεί τη ζητούμενη εργασία (αν υπάρχει) από το job buffer.
    ...
    4.5. Η συνάρτηση «char* exit_server()»:
        ...
        - Είναι υπεύθυνη για την αποστολή μηνήματος για το πρώιμο κλείσμο του Server, σε όλους τους 
        Commander (αν υπάρχουν) που δεν έχει αρχίσει να εκτελείται το job που έδωσαν στον Server.
        - Είναι υπέυθυνη για τη σήμανση τους κλεισίματος του Server.
